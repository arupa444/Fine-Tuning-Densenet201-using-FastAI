...
async def predict_marker(
    ...
    try:
        ...
        for ind, crate_box in enumerate(crate_results[0].obb.xyxy):
            ...
            marker_np_bgr = cv2.cvtColor(marker_np, cv2.COLOR_RGB2BGR)
            marker_letterboxed = letterbox_for_marker_crop(marker_np, target_size=640)
            marker_results = marker_model.predict(source=marker_letterboxed, conf=0.5, verbose=False)
